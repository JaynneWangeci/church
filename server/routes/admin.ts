...
// ── General member fellowship suggestions ──
adminRouter.get("/general-member-analysis", requireAdmin, async (_req, res) => {
  try {
    const db = requireService();

    // 1. Get all general members
    const { data: generalMembers } = await db
      .from("church_members")
      .select("id, name, phone, created_at")
      .eq("is_active", true)
      .eq("council", "general_member")
      .order("name");

    if (!generalMembers?.length) return res.json({ members: [] });

    // 2. Get all non-general members for surname matching
    const { data: fellowshipMembers } = await db
      .from("church_members")
      .select("id, name, council")
      .eq("is_active", true)
      .neq("council", "general_member");

    // Build surname → council map from fellowship members
    const surnameCouncil = new Map<string, { council: string; examples: string[] }>();
    for (const m of fellowshipMembers || []) {
      const parts = m.name.trim().split(/\s+/);
      const surname = parts[parts.length - 1].toLowerCase();
      if (!surnameCouncil.has(surname)) {
        surnameCouncil.set(surname, { council: m.council, examples: [] });
      }
      if (surnameCouncil.get(surname)!.examples.length < 3) {
        surnameCouncil.get(surname)!.examples.push(m.name);
      }
    }

    // 3. Get pledges and donations for general members
    const pledgeMap = new Map<string, { count: number; total: number }>();
    const donationMap = new Map<string, { count: number; total: number }>();
    ...
    for (const p of allPledges || []) {
      ...
      const cur = pledgeMap.get(key) || { count: 0, total: 0 };
      cur.count++;
      cur.total += Number(p.amount);
      pledgeMap.set(key, cur);
    }

    for (const d of allDonations || []) {
      const key = (d.donor_name || "").toLowerCase().trim();
      if (!key) continue;
      const cur = donationMap.get(key) || { count: 0, total: 0 };
      cur.count++;
      cur.total += Number(d.amount);
      donationMap.set(key, cur);
    }

    // 4. Build analysis for each general member
    const analysis = [];
    for (const gm of generalMembers) {
      const nameLower = gm.name.toLowerCase().trim();
      const parts = gm.name.trim().split(/\s+/);
      const surname = parts.length > 1 ? parts[parts.length - 1].toLowerCase() : null;

      const pledges = pledgeMap.get(nameLower) || { count: 0, total: 0 };
      const donations = donationMap.get(nameLower) || { count: 0, total: 0 };

      // Try surname match for fellowship suggestion
      let suggestedCouncil: string | null = null;
      let matchedByName: string[] = [];
      if (surname && surnameCouncil.has(surname)) {
        const match = surnameCouncil.get(surname)!;
        suggestedCouncil = match.council;
        matchedByName = match.examples;
      }

      // Also check if this name appears in a fellowship member's house
      const household = (fellowshipMembers || []).filter(m => {
        const mParts = m.name.trim().split(/\s+/);
        const mSurname = mParts[mParts.length - 1].toLowerCase();
        return surname && mSurname === surname && m.name.toLowerCase().trim() !== nameLower;
      });
      ...
      analysis.push({
        ...
        transactions: {
          pledges: pledges.count > 0 ? pledges : null,
          donations: donations.count > 0 ? donations : null,
        },
        surname_match: suggestedCouncil
          ? { suggested_council: suggestedCouncil, matched_names: matchedByName }
          : null,
        household: household.length > 0
          ? household.map(m => ({ name: m.name, council: m.council }))
          : [],
      });
    }

    // Sort: members with pledges/donations first, then surname matches, then rest
    analysis.sort((a, b) => {
      const aScore = (a.transactions.pledges ? 2 : 0) + (a.transactions.donations ? 2 : 0) + (a.surname_match ? 1 : 0);
      const bScore = (b.transactions.pledges ? 2 : 0) + (b.transactions.donations ? 2 : 0) + (b.surname_match ? 1 : 0);
      return bScore - aScore;
    });

    res.json({
      total_general_members: generalMembers.length,
      members_with_transactions: analysis.filter(m => m.transactions.pledges || m.transactions.donations).length,
      members_with_surname_match: analysis.filter(m => m.surname_match).length,
      members: analysis,
    });
  } catch (err) {
    console.error("general member analysis error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
